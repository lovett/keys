...
The application can be run with a systemd user service generated by running `keys --systemd`.
...
Run `keys --soundtest` to verify that audio is working correctly.

Run `keys --keytest` to see the name of a pressed key. For letter and number keys this will probably be what you expect, but function and multimedia and numpad keys can be exotic and hardware-specific.
